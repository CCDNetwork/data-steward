...
export const resToAttributeGroup = (res: any): AttributeGroup => {
  return {
    ...
    beneficiaryAttributes: res.beneficiaryAttributes ? res.beneficiaryAttributes.map(resToBeneficiaryAttribute) : [],
...
export const attributeGroupToReq = (data: any): Omit<AttributeGroup, 'id' | 'createdAt' | 'updatedAt'> => {
  const req: any = {
    title: data.title,
    name: data.name,
